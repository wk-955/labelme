...
class CalculationBisectionPoints:
    ...
    def inputPoint(self, points, num):
        self.spacingPoints = [points[0]]
        self.distanceArr = []
        self.distanceSum = 0
        self.surplusDistance = 0

        long = self.calculateDistanceSum(points)
        self.pointArr = points
        self.spacing = long / num
        self.doCalculate()

        if len(list(self.spacingPoints)) != num+1:
            result = list(self.spacingPoints) + [points[-1]]
            return result
        return self.spacingPoints